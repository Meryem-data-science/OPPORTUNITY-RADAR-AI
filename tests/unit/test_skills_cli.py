"""Unit coverage for the local Phase 3.4A skill projection CLI.

Every database is created under `tmp_path` and thrown away; the operational
`.data/` database is never opened. The skill values below are invented
technology names and the address is a `.invalid` one that never resolves.

This command is the opposite of the Phase 3.3B review: it needs nobody to read
a value, so it prints none. Half of these tests are about that — the summary,
the structured log and the error messages carry counters, versions and ids and
nothing else.
"""
...
from services.digital_twin.skills import cli
...
#: Invented skill mentions. They must never leave the database.
TEST_ONLY_SKILLS = ("PowerBI", "Power BI", "Azure Data Platform (avancé)", "C++")
...
def _seed(path, values=TEST_ONLY_SKILLS) -> None:
...
def database(monkeypatch, tmp_path):
    """A disposable SQLite database holding one profile and its skill facts."""
...
def skills_of(path) -> list[str]:
    connection = connect_database(path)
    try:
        return [skill.canonical_key for skill in list_profile_skills(connection, 1)]
    finally:
        connection.close()
...
def test_sync_projects_the_accepted_facts_and_reports_counters(
    ...
    assert exit_code == 0
    assert summary["verified_skill_facts"] == "4"
    assert summary["profile_skills"] == "3"
    assert summary["evidence_created"] == "4"
    assert summary["normalizer_version"] == "skill-normalizer-v1"
    assert summary["changed"] == "true"
    assert sorted(skills_of(database)) == [
        "azure data platform (avancé)",
        "c++",
        "power bi",
    ]
...
def test_a_second_run_reports_no_change(silence_logging, database, capsys) -> None:
    ...
    assert summary["profile_skills_created"] == "0"
...
def test_the_summary_names_no_skill(silence_logging, database, capsys) -> None:
    cli.main([cli.SYNC_COMMAND, "--email", TEST_ONLY_EMAIL])
    output = capsys.readouterr().out

    for value in TEST_ONLY_SKILLS:
        assert value not in output
    assert "Power BI" not in output
    assert TEST_ONLY_LOCAL_PART not in output


def test_structured_events_carry_counters_and_never_a_skill(database, capsys) -> None:
    ...
    succeeded = next(
        event for event in events if event["event"] == "profile_skill_sync_succeeded"
    )
    assert succeeded["context"] == {
        "database_backend": "sqlite",
        "verified_skill_facts": 4,
        "profile_skills": 3,
        "skills_created": 3,
        "profile_skills_created": 3,
...
def test_the_command_offers_no_flag_that_would_print_a_skill() -> None:
    """The privacy of the output is a property of the command, not of a default."""
    parser_help = cli.parse_args([cli.SYNC_COMMAND]).__dict__

    assert set(parser_help) == {"command", "email"}
...
def test_a_missing_profile_is_reported_and_never_created(
    ...
    """Synchronizing skills is not a way to bring a profile into existence."""
...
def test_only_the_configured_database_is_written(
    ...
    assert skills_of(untouched) == []
    assert skills_of(configured) != []
