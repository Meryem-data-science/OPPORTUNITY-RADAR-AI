...
from services.collector.models.source_run import FAILED, SUCCESS
...
def test_successful_run_is_persisted_with_exact_counts_and_stamps_last_run_at(tmp_path):
    path, settings = migrated_settings(tmp_path, "success.db")
    collected = [candidate("board_a", "1"), candidate("board_a", "2")]
...
def test_failed_run_is_persisted_and_never_stores_a_credential(tmp_path):
    ...
    summary = RadarAgent(
        source_loader=lambda: [source("board_a")],
        collector_factory=lambda unused: FakeCollector(error=error),
        settings_loader=lambda: settings,
    ).run_once()
...
def test_repeated_executions_are_distinct_history_rows_and_are_never_deduplicated(tmp_path):
    ...
    collected = [candidate("board_a")]
    failure = FakeCollector(error=TimeoutError("board timed out"))
    outcomes = [FakeCollector(collected), failure, FakeCollector(collected)]
