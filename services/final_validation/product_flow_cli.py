"""Phase 11.2C CLI: print read-only evidence that persisted state reaches the product read paths.

    DATABASE_BACKEND=sqlite \\
    SQLITE_DATABASE_PATH=<existing SQLite file> \\
    OPPORTUNITY_RADAR_PROFILE_ID=<positive integer> \\
    python -m services.final_validation.product_flow_cli

Nothing is defaulted: the backend, the path and the profile must all be stated.
The evidence goes to stdout as deterministic JSON. Nothing is sent, requested,
saved or synchronized, and no opportunity URL is contacted.
...
from services.final_validation.product_flow import SCHEMA_VERSION, validate_product_flow
...
def main(
    ...
    validate: Callable[..., dict] = validate_product_flow,
) -> int:
    stdout = sys.stdout if stdout is None else stdout
    stderr = sys.stderr if stderr is None else stderr
    # The product read paths log a refusal through the shared service logger,
    # which writes to stdout by default. Stdout carries only the evidence JSON.
    configure_logging(stderr)
