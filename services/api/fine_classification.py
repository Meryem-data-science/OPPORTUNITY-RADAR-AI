...
def _check_coherence(
    ...
    evidence: list[FineEvidenceResponse],
) -> None:
...
def decode_fine_classification(
    ...
    `None` six times and reads as state A, which is why the `LEFT JOIN` needs no
    ...
    if qualification is None:
        # State A, reached by the LEFT JOIN matching no qualification row: every
    ...
    # The stored version is read, never compared to the version running now: the
    # API reports versioned persisted data and must not recompute it.
